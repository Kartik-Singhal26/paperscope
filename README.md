...
🖼️ **Poster Studio**: one click turns any paper into an editable conference poster — three templates (neo-brutal / classic / minimal), A0 portrait & landscape and 48×36 in sizes, and print-grade vector PDF export via the browser's print dialog. Sections are auto-drafted from the paper's *public* abstract (structured abstracts become real Background/Methods/Results blocks); when the abstract is closed-access the studio says so honestly and hands you an editable block instead of inventing text. Every text block is click-to-edit, figures upload straight from your files, and the poster carries live impact stats, citing-country flags, funder credits, key references, and two QR codes (paper DOI + live PaperScope analytics) generated by a built-in, dependency-free QR encoder.
